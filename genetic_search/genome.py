...
class Uniform:
    ...
    def mutate(self, value, rate=0.2):
        ...
        return min(self.high, max(self.low, value + random.uniform(-delta, delta)))
...
class Genome:
    parameter_space: Dict[str, Union[Uniform, IntRange, Choice]] = {
        ...
        "entropy_coef_init": Uniform(0.01, 0.1),
        "entropy_coef_final": Uniform(0.001, 0.01),
        "gae_lambda": Uniform(0.90, 0.999),
        "gamma": Uniform(0.90, 0.999),
        "clip_range": Uniform(0.1, 0.3),
        "batch_size": Choice([32, 64, 128, 256]),

        # Reward shaping
        "sl_penalty_coef": Uniform(5.0, 20.0),
        "tp_reward_coef": Uniform(5.0, 20.0),
        "timeout_duration": IntRange(1, 10),
        "ongoing_reward_coef": Uniform(0.1, 1.0)
    }
